#################################
#
# File Name:         pppoe_dhcpv6pd_server_client.robot
#
...
#
# Run sample:
#            c:\>robot pppoe_dhcpv6pd_server_client.robot
...
*** Test Cases ***
pppoe dhcpv6pd server client test
    [Documentation]  pppoe dhcpv6pd server client test
...
##############################################################
    ...
    ${test_sta} =  test config  log=1   logfile=pppoe_dhcpv6pd_server_client_logfile   vendorlogfile=pppoe_dhcpv6pd_server_client_stcExport   vendorlog=1   hltlog=1   hltlogfile=pppoe_dhcpv6pd_server_client_hltExport   hlt2stcmappingfile=pppoe_dhcpv6pd_server_client_hlt2StcMapping   hlt2stcmapping=1   log_level=7
...
##############################################################
    ...
    ${device_ret0} =  pppox server config  mode=create   encap=ethernet_ii   protocol=pppoe   ipv6_pool_prefix_step=2::   ipv6_pool_intf_id_start=::1   ipv6_pool_intf_id_step=::2   ipv6_pool_addr_count=50   ipv6_pool_prefix_len=64   ipv6_pool_prefix_start=1000::   port_handle=${port1}   max_outstanding=100   disconnect_rate=1000   attempt_rate=100   enable_osi=false   pap_req_timeout=3   mru_neg_enable=1   max_configure_req=10   term_req_timeout=3   max_terminate_req=10   username=viavi   force_server_connect_mode=false   echo_vendor_spec_tag_in_pado=false   echo_vendor_spec_tag_in_pads=false   max_payload_tag_enable=false   max_ipcp_req=10   echo_req_interval=10   config_req_timeout=3   local_magic=1   password=viavi   chap_reply_timeout=3   max_chap_req_attempt=10   enable_mpls=false   lcp_mru=1492   ip_cp=ipv6_cp   max_echo_acks=1   auth_mode=none   include_id=1   ipcp_req_timeout=3   server_inactivity_timer=30   unconnected_session_threshold=0   max_payload_bytes=1500   echo_req=false   fsm_max_naks=5   gateway_ipv6_step=::   intf_ipv6_addr=2000::2   intf_ipv6_addr_step=::2   intf_ipv6_prefix_length=64   gateway_ipv6_addr=2000::1   mac_addr=00:10:94:01:00:01   mac_addr_step=00:00:00:00:00:01   num_sessions=1
...
###############################################################
    ...
    ${results_ret6} =  emulation dhcp stats  port_handle=${port2}   action=collect   mode=session   ip_version=6
